...
SyncR = TypeVar("SyncR", bound=base_alchemy_repository.BaseAlchemySyncRepository)
AsyncR = TypeVar("AsyncR", bound=base_alchemy_repository.BaseAlchemyAsyncRepository)


class BaseAlchemySyncUOW(base_uow.BaseSyncUOW, Generic[SyncR]):
    """
    UOW для работы с синхронными репозиториями Алхимии
    """

    def __init__(self, repository: SyncR) -> None:
        """
    ...
    def __enter__(self) -> BaseAlchemySyncUOW[SyncR]:
        """
...
class BaseAlchemyAsyncUOW(base_uow.BaseAsyncUOW, Generic[AsyncR]):
    """
    UOW для работы с асинхронными репозиториями Алхимии
    """

    def __init__(self, repository: AsyncR) -> None:
        """
        Инициализировать переменные
    ...
    async def __aenter__(self) -> BaseAlchemyAsyncUOW[AsyncR]:
        """
        Войти в контекстный менеджер
        :return: объект UOW
        """
...
class TestBaseAlchemySyncUOW(BaseAlchemySyncUOW[SyncR], Generic[SyncR]):
    """
    Тестовый UOW для синхронной реализации Алхимии
    """

    def __init__(self, repository: SyncR):
        """
        Инициализировать переменные
    ...
    def __enter__(self) -> TestBaseAlchemySyncUOW[SyncR]:
        """
        Войти в контекстный менеджер
        :return: объект UOW
        """
...
class TestBaseAlchemyAsyncUOW(BaseAlchemyAsyncUOW[AsyncR], Generic[AsyncR]):
    """
    Тестовый UOW для асинхронной реализации Алхимии
    """

    def __init__(self, repository: AsyncR):
        """
        Инициализировать переменные
        :param repository: репозиторий
        """
    ...
    async def __aenter__(self) -> TestBaseAlchemyAsyncUOW[AsyncR]:
        """
        Войти в контекстный менеджер
        :return: объект UOW
        """
