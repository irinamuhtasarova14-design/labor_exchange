...
Предоставляется интерфейс **ConnectionProxy** из **bases/base_proxy.py** для синхронных и асинхронных подключений к базе данных PostgreSQL. Прокси-соединение принимает в качестве аргументов фабрику движков для подключения к БД.
...
Предлагается реализация репозитория, совместимого как с синхронными, так и с асинхронными подходами, который реализует интерфейс **BaseRepository** из **bases/repositories/base_repository.py**.
Базовый класс репозитория находится в директории **bases/repositories/base_alchemy_repository.py**.
...
Готовые тестовые UOW реализованы для синхронного (**TestAlchemySyncUOW**) и асинхронного (**TestAlchemyAsyncUOW**) взаимодействия и находятся в модуле **bases/uows/generic_alchemy_uow.py**.
