...
Если приложение находится в режиме разработки (dev-stage), то корневой логгер работает с фильтром **info** (уровень INFO и выше), а логгер uvicorn — с фильтром **debug** (все уровни). В остальных режимах фильтры применяются в обратном порядке.
